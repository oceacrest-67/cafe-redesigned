document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Cinematic Hero Slideshow (Ken Burns + Velvet Cross-fade) ---
        ...
        const playSlideshow = () => {
            ...
            const tl = gsap.timeline({
                onComplete: () => {
                    ...
                    gsap.delayedCall(4, playSlideshow); // Wait 4 seconds before next
                }
            });

            // 1. Velvet Cross-fade
            tl.to(current, { opacity: 0, duration: 2, ease: "sine.inOut" }, 0);
            tl.fromTo(next, 
                { opacity: 0, scale: 1.2, zIndex: 2 }, 
                { opacity: 1, scale: 1.1, duration: 2, ease: "sine.inOut" }, 
                0
            );

            // 2. The Ken Burns "Slow Zoom"
            // While the slide is active (between fades), it continues to zoom in
            gsap.to(next, { scale: 1, duration: 6, ease: "none" });
        };

        // Start initial slide zoom
        gsap.to(slides[0], { scale: 1, duration: 6, ease: "none" });
        gsap.delayedCall(4, playSlideshow); // Kick off the loop
    }
